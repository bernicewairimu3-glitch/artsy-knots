...
const Render={
  ...
  copy(){const c=Store.state.copy;
    ...
    if(heroImg){heroImg.innerHTML=c.heroImage?'<img src="'+esc(c.heroImage)+'" alt="'+esc(c.heroTitle)+'">':'';heroImg.style.display=c.heroImage?'block':'none';}
...
const Gallery={
  ...
  buildFilters(){const items=Store.state.products;const counts={};items.forEach(function(p){counts[p.category]=(counts[p.category]||0)+1;});
    ...
    const labels={all:'All works',matwork:'Mats',bags:'Bags',canvas:'Paintings',resin:'Resin',commission:'Commissions'};
...
const Edit={
  ...
  heroImageFile(ev){const f=ev.target.files[0];if(!f)return;const r=new FileReader();r.onload=function(){Store.state.copy.heroImage=r.result;document.getElementById('f_heroImage').value='';Render.copy();Store.save();UI.toast('Hero image updated');};r.readAsDataURL(f);},
  copy(){const c=Store.state.copy;c.heroEyebrow=this.v('f_heroEyebrow');c.heroTitle=this.v('f_heroTitle');c.heroLede=this.v('f_heroLede');if(this.v('f_heroImage'))c.heroImage=this.v('f_heroImage');c.aboutHeading=this.v('f_aboutHeading');c.aboutBy=this.v('f_aboutBy');c.aboutBody=this.v('f_aboutBody');c.aboutQuote=this.v('f_aboutQuote');if(this.v('f_aboutPortrait'))c.aboutPortrait=this.v('f_aboutPortrait');c.beliefs=this.v('f_beliefs');c.contactHeading=this.v('f_contactHeading');c.contactBody=this.v('f_contactBody');Render.copy();dSave();},
